...
\texttt{followees} array is the last one,
\texttt{\textquotesingle@TheEllenShow\textquotesingle}, which is exactly 13
characters long.) The ``\texttt{float64}'' thing means ``\texttt{float}s, each
of which is represented with 64 bits\footnote{A ``bit'' -- which is short for
``binary digit'' -- is the tiniest piece of information a computer can store:
it's a single 0 or 1.} in memory.
...
for elements in the range of 1776 to 2025, and we got...1776 through
\textit{2024}. Huh?
...
Notice we had to specify 2010 and 2028 as the second argument to these function
calls in order for the arrays to include 2000, and 2024, respectively. This is
...
think you have a file called ``\texttt{nfl2025}'' when you actually have one
called ``\texttt{nfl2025.txt}'' or ``\texttt{nfl2025.csv},'' and Windows thinks
